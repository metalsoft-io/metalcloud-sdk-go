...
func (r ExtensionInstanceAPIGetExtensionInstanceCredentialsRequest) Execute() (*ExtensionInstanceCredentials, *http.Response, error) {
	return r.ApiService.GetExtensionInstanceCredentialsExecute(r)
}

/*
GetExtensionInstanceCredentials Get Extension Instance credentials

Returns the credentials of the specified Extension Instance
...
// Execute executes the request
//  @return ExtensionInstanceCredentials
func (a *ExtensionInstanceAPIService) GetExtensionInstanceCredentialsExecute(r ExtensionInstanceAPIGetExtensionInstanceCredentialsRequest) (*ExtensionInstanceCredentials, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *ExtensionInstanceCredentials
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ExtensionInstanceAPIService.GetExtensionInstanceCredentials")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/extension-instances/{extensionInstanceId}/credentials"
	localVarPath = strings.Replace(localVarPath, "{"+"extensionInstanceId"+"}", url.PathEscape(parameterValueToString(r.extensionInstanceId, "extensionInstanceId")), -1)
...
// Filter by extensionId query param.  **Format:** filter.extensionId&#x3D;{$not}:OPERATION:VALUE    **Example:** filter.extensionId&#x3D;$eq:John Doe  **Available Operations** - $eq  - $and  - $or
...
// Parameter to sort by. To sort by multiple fields, just provide query param multiple types. The order in url defines an order of sorting  **Format:** {fieldName}:{DIRECTION}   **Example:** sortBy&#x3D;id:DESC&amp;sortBy&#x3D;label:DESC   **Default Value:** id:DESC  **Available Fields** - id  - label  - extensionId  - infrastructureId 
